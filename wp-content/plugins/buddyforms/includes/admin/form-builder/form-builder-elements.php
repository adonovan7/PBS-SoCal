...
function buddyforms_display_form_element( $args )
{
    ...
    $form_fields['validation']['validation_error_message'] = new Element_Textbox( '<b>' . __( 'Validation Error Message', 'buddyforms' ) . '</b>', "buddyforms_options[form_fields][" . $field_id . "][validation_error_message]", array(
    ...
    switch ( sanitize_title( $field_type ) ) {
        ...
        case 'textarea':
            ...
            $form_fields['advanced']['hidden'] = new Element_Checkbox(
                '<b>' . __( 'Hidden?', 'buddyforms' ) . '</b>',
                "buddyforms_options[form_fields][" . $field_id . "][hidden]",
                array(
                'hidden' => '<b>' . __( 'Make this field Hidden', 'buddyforms' ) . '</b>',
            ),
                array(
                'value' => $hidden,
            )
        ...
        case 'number':
            ...
            $form_fields['validation']['validation_min'] = new Element_Number( '<b>' . __( 'Validation Min Length', 'buddyforms' ) . '</b>', "buddyforms_options[form_fields][" . $field_id . "][validation_min]", array(
                'value' => $validation_min,
            ) );
            $validation_max = ( isset( $customfield['validation_max'] ) ? stripcslashes( $customfield['validation_max'] ) : 0 );
            $form_fields['validation']['validation_max'] = new Element_Number( '<b>' . __( 'Validation Max Length', 'buddyforms' ) . '</b>', "buddyforms_options[form_fields][" . $field_id . "][validation_max]", array(
...
JS;
        ...
        case 'comments':
            unset( $form_fields );
            $required = ( isset( $customfield['required'] ) ? $customfield['required'] : 'false' );
            $form_fields['general']['required'] = new Element_Checkbox(
                '<b>' . __( 'Required', 'buddyforms' ) . '</b>',
                "buddyforms_options[form_fields][" . $field_id . "][required]",
                array(
                'required' => '<b>' . __( 'Required', 'buddyforms' ) . '</b>',
            ),
                array(
                'value' => $required,
                'id'    => "buddyforms_options[form_fields][" . $field_id . "][required]",
    ...
    echo  $field_id ;
    ?> bf_<?php 
    echo  sanitize_title( $field_type ) ;
    ?>" data-field_id="<?php 
    echo  $field_id ;
    ?> ">

    <input id="this_field_id" type="hidden" value="<?php 
    echo  $field_id ;
    ?>">
    ...
    ?>
                        ...
                        <td class="field_layout">
                            <select class="" name="buddyforms_options[layout][cords][<?php 
    echo  $field_id ;
    ?>]">
                                <option <?php 
    selected( $layout, '1' );
    ?> value="1">Full Width</option>
                                <option <?php 
    selected( $layout, '2' );
    ?> value="2">1/2</option>
                                <option <?php 
...
function buddyforms_form_element_multiple( $form_fields, $args )
{
    ...
    if ( isset( $buddyform['form_fields'][$field_id]['options'] ) ) {
        ...
        foreach ( $buddyform['form_fields'][$field_id]['options'] as $key => $option ) {
            ...
            $form_element = new Element_Textbox( '', "buddyforms_options[form_fields][" . $field_id . "][options][" . $key . "][label]", array(
                'value' => $option['label'],
            ) );
            $form_element->render();
            echo  '</td><td>' ;
            $form_element = new Element_Textbox( '', "buddyforms_options[form_fields][" . $field_id . "][options][" . $key . "][value]", array(
                'value' => $option['value'],
            ) );
            $form_element->render();
            echo  '</td><td>' ;
            $form_element = new Element_Radio(
                '',
                "buddyforms_options[form_fields][" . $field_id . "][default]",
                array( $option['value'] ),
                array(
                'value' => ( isset( $buddyform['form_fields'][$field_id]['default'] ) ? $buddyform['form_fields'][$field_id]['default'] : '' ),
            )
            );
            $form_element->render();
...
function buddyforms_form_element_gdpr( $form_fields, $args )
{
    ...
    if ( isset( $buddyform['form_fields'][$field_id]['options'] ) ) {
        ...
        foreach ( $buddyform['form_fields'][$field_id]['options'] as $key => $option ) {
            ...
            $form_element = new Element_Textarea( '', "buddyforms_options[form_fields][" . $field_id . "][options][" . $key . "][label]", array(
                'value' => $option['label'],
                'cols'  => '50',
            ) );
            $form_element->render();
            ...
            $form_element = new Element_Textarea( '', "buddyforms_options[form_fields][" . $field_id . "][options][" . $key . "][error_message]", array(
                'value' => $error_message,
                'cols'  => '50',
                'rows'  => '2',
            ) );
            ...
            $form_element = new Element_Checkbox(
                '',
                "buddyforms_options[form_fields][" . $field_id . "][options][" . $key . "][checked]",
                array(
                'checked' => 'Checked',
            ),
                array(
            ...
            $form_element = new Element_Checkbox(
                '',
                "buddyforms_options[form_fields][" . $field_id . "][options][" . $key . "][required]",
                array(
                'required' => 'Required',
            ),
                array(
...
{
    ?>
    <table class="wp-list-table widefat posts fixed <?php 
    echo  $striped ;
    ?>">
        <tbody>
		<?php 
