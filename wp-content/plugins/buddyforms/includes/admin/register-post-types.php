...
function buddyforms_add_meta_boxes()
{
    ...
    if ( buddyforms_core_fs()->is_not_paying() && !buddyforms_core_fs()->is_trial() ) {
        add_meta_box(
            'buddyforms_form_go_pro',
            __( "Awesome Premium Features", 'buddyforms' ),
            'buddyforms_metabox_go_pro',
            'buddyforms',
            'side',
            'low'
        );
    ...
    if ( is_array( $buddyform ) ) {
        add_meta_box(
            'buddyforms_form_shortcodes',
            __( "Shortcodes", 'buddyforms' ),
            'buddyforms_metabox_shortcodes',
            'buddyforms',
            'side',
            'low'
        );
    ...
    add_meta_box(
        'buddyforms_form_elements',
        __( "Form Builder", 'buddyforms' ),
        'buddyforms_metabox_form_elements',
        'buddyforms',
        'normal',
        'high'
    );
    add_meta_box(
        'buddyforms_form_setup',
        __( "Form Setup", 'buddyforms' ),
        'buddyforms_metabox_form_setup',
        'buddyforms',
        'normal',
        'high'
    );
    add_meta_box(
        'buddyforms_form_designer',
        __( "Form Designer", 'buddyforms' ),
        'buddyforms_metabox_form_designer',
        'buddyforms',
        'normal',
        'high'
    );
    // NinjaForms jQuery dialog is different from core so we remove the NinjaForms media buttons on the BuddyForms views
    buddyforms_remove_filters_for_anonymous_class(
        'media_buttons_context',
        'NF_Admin_AddFormModal',
        'insert_form_tinymce_buttons',
        10
    );
}
...
add_action(
    ...
    'buddyforms_transition_post_status_regenerate_global_options',
    10,
    3
);
/**
...
add_filter(
    'manage_buddyforms_posts_columns',
    'set_custom_edit_buddyforms_columns',
    10,
    1
);
/**
    ...
    switch ( $column ) {
        ...
        case 'attached_page':
            ...
            if ( isset( $buddyform['attached_page'] ) && empty($buddyform['attached_page']) ) {
                ...
                $attached_page = '<p>' . __( 'On', 'buddyforms' ) . '</p>';
                // . '<br>' . $attached_page_title . '</p>';
            } else {
                $attached_page = 'Off';
            }
            ...
            if ( $attached_page != 'Off' ) {
                ...
                echo  $attached_page_permalink . 'create/' . $post->post_name ;
                ?>">View
							Form</a> |
					</span>
                    <span class="view-entryies">
						<a target="_blank" href="<?php 
                echo  $attached_page_permalink . 'view/' . $post->post_name ;
                ?>">View
							Entries</a>
					</span>
...
add_action(
    'manage_buddyforms_posts_custom_column',
    'custom_buddyforms_column',
    10,
    2
);
/**
...
function buddyforms_add_button_to_submit_box()
{
    ...
    if ( isset( $post->post_name ) && $post->post_name != '' ) {
        ...
        echo  $base ;
        ?>/?page_id=<?php 
        echo  $preview_page_id ;
        ?>&preview=true&form_slug=<?php 
        echo  $post->post_name ;
        ?>"><span
                            class="dashicons dashicons-visibility"></span> <?php 
        _e( 'Preview Form', 'buddyforms' );
        ?>
                </a>
            </div>
		<?php 
    ...
    if ( isset( $buddyform['attached_page'] ) && isset( $buddyform['post_type'] ) && $buddyform['attached_page'] != 'none' ) {
        ?>
            <div id="frontend-actions">
				...
				<?php 
        echo  '<a class="button button-large bf_button_action" href="' . $attached_page_permalink . 'view/' . $post->post_name . '/" target="_new"><span class="dashicons dashicons-admin-page"></span> ' . __( 'Your Submissions', 'buddyforms' ) . '</a>
    ...
    if ( isset( $post->post_name ) && $post->post_name != '' ) {
        ?>
            <div id="admin-actions">
				...
				<?php 
        echo  '<a class="button button-large bf_button_action" href="edit.php?post_type=buddyforms&page=buddyforms_submissions&form_slug=' . $post->post_name . '"><span class="dashicons dashicons-email"></span> ' . __( 'Submissions', 'buddyforms' ) . '</a>' ;
        ?>
            </div>
		<?php 
    }
...
function buddyforms_notice_if_broken_form()
{
    ...
    if ( isset( $buddyform['form_fields'] ) ) {
        foreach ( $buddyform['form_fields'] as $field_key => $field ) {
            if ( $field['type'] == 'taxonomy' || $field['type'] == 'category' || $field['type'] == 'tags' || $field['type'] == 'featured_image' ) {
    ...
    <font color="#b22222"><?php 
    _e( 'This Form is Broken!', 'buddyforms' );
    ?></font>
	<?php 
