...
if ( !class_exists( 'BuddyForms' ) ) {
    ...
    class BuddyForms
    {
        ...
        function admin_styles( $hook_suffix )
        {
            ...
            if ( isset( $post ) && $post->post_type == 'buddyforms' && isset( $_GET['action'] ) && $_GET['action'] == 'edit' || isset( $post ) && $post->post_type == 'buddyforms' && $hook_suffix == 'post-new.php' || $hook_suffix == 'buddyforms_page_buddyforms_submissions' || $hook_suffix == 'buddyforms_page_buddyforms_settings' ) {
                if ( is_rtl() ) {
        ...
        function admin_js( $hook_suffix )
        {
            ...
            if ( isset( $post ) && $post->post_type == 'buddyforms' && isset( $_GET['action'] ) && $_GET['action'] == 'edit' || isset( $post ) && $post->post_type == 'buddyforms' && $hook_suffix == 'post-new.php' || $hook_suffix == 'buddyforms_page_buddyforms_submissions' || $hook_suffix == 'buddyforms_page_buddyforms_settings' ) {
                ...
                wp_register_script(
                    'buddyforms-admin-slugifies-js',
                    plugins_url( 'assets/admin/js/slugifies.js', __FILE__ ),
                    array(),
                    $this->version
                );
                wp_register_script(
                    'buddyforms-admin-wizard-js',
                    plugins_url( 'assets/admin/js/wizard.js', __FILE__ ),
                    array(),
                    $this->version
                );
                wp_register_script(
                    'buddyforms-admin-deprecated-js',
                    plugins_url( 'assets/admin/js/deprecated.js', __FILE__ ),
                    array(),
                    $this->version
                );
                wp_register_script(
                    'buddyforms-admin-conditionals-js',
                    plugins_url( 'assets/admin/js/conditionals.js', __FILE__ ),
                    array(),
                    $this->version
                );
                wp_register_script(
                    'buddyforms-admin-formbuilder-js',
                    plugins_url( 'assets/admin/js/formbuilder.js', __FILE__ ),
                    array(),
                    $this->version
                );
                // GDPR Localisation
                ...
                wp_enqueue_script(
                    'buddyforms-jquery-steps-js',
                    plugins_url( 'assets/resources/jquery-steps/jquery.steps.min.js', __FILE__ ),
                    array( 'jquery' ),
                    $this->version
                );
            ...
            wp_enqueue_script(
                'buddyforms-admin-all-js',
                plugins_url( 'assets/admin/js/admin-all.js', __FILE__ ),
                array( 'jquery' ),
                $this->version
            );
            wp_enqueue_media();
            ...
            wp_enqueue_script(
                'buddyforms_dropzone_initializer',
                plugins_url( 'assets/resources/dropzone/initializer.js', __FILE__ ),
                array( 'jquery' ),
                $this->version,
                true
            );
            wp_enqueue_style(
                'buddyforms-dropzone-basic',
                plugins_url( 'assets/resources/dropzone/basic.css', __FILE__ ),
                array(),
                $this->version
            );
            wp_enqueue_style(
                'buddyforms-dropzone',
                plugins_url( 'assets/resources/dropzone/dropzone.css', __FILE__ ),
                array(),
                $this->version
            );
            $params = array(
            ...
            //Featured image
            wp_enqueue_script(
                'buddyforms_featured_image_initializer',
                plugins_url( 'assets/resources/featured-image/featured-image-initializer.js', __FILE__ ),
                array( 'jquery' ),
                $this->version
            );
        ...
        function front_js_css()
        {
            ...
            wp_enqueue_script(
                'buddyforms-select2-js',
                plugins_url( 'assets/resources/select2/dist/js/select2.min.js', __FILE__ ),
                array( 'jquery' ),
                '4.0.3'
            );
            wp_enqueue_style( 'buddyforms-select2-css', plugins_url( 'assets/resources/select2/dist/css/select2.min.css', __FILE__ ) );
            wp_enqueue_script(
                ...
                plugins_url( 'assets/resources/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.js', __FILE__ ),
            ...
            wp_enqueue_script(
                'buddyforms-gdpr-js',
                plugins_url( 'assets/js/gdpr.js', __FILE__ ),
                array( 'jquery' ),
                $this->version,
                false
            );
        ...
        public function admin_footer_text( $footer_text )
        {
            ...
            if ( $current_screen->id == 'edit-buddyforms' || $current_screen->id == 'buddyforms' || $current_screen->id == 'buddyforms_page_buddyforms_submissions' || $current_screen->id == 'buddyforms_page_buddyforms_settings' || $current_screen->id == 'buddyforms_page_bf_add_ons' ) {
